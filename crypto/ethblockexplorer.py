    ...
    async def parse_command(self, message, client):
        ...
        if len(msg) > 2: # !eth <type> <hash>
            if msg[1] == 'tx':
                ...
                embed.add_field(name="Value", value=str(self.wei_to_eth(data["value"])) + " ETH", inline=True)
                embed.add_field(name="Gas Limit", value=self.comma_money(int(data["gas"])), inline=True)
